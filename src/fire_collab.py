...
def planner_node(state: CollaborativeAgentState) -> Dict[str, Any]:
            ...
            {{
                "plan": [
                    {{
                        ...
                        "description": "Pesquise os avanços mais recentes e significativos na tecnologia de carros autônomos nos últimos 1-2 anos. Foque em inovações chave, desafios e empresas líderes."
                    }},
                    {{
                        "task_id": "write_summary_autonomous_cars",
                        "specialist_type": "writer",
                        "description": "Com base na pesquisa sobre carros autônomos (especialmente os resultados de 'research_autonomous_cars'), escreva um resumo conciso (2-3 parágrafos) dos avanços recentes. Destaque os pontos mais importantes de forma clara e acessível."
                    }}
                ]
            }}
        """
    ...
    try:
        response = models["gpt_3_5"].invoke([system_message_planner, human_message])
...
def researcher_node(state: CollaborativeAgentState) -> Dict[str, str]:
    ...
    if scraped_content_for_llm and "Erro ao tentar obter conteúdo da web" not in scraped_content_for_llm and "Nenhum conteúdo web pôde ser obtido" not in scraped_content_for_llm :
    ...
    try:
        response = models["gpt_3_5"].invoke([system_message_researcher, human_message])
...
def writer_node(state: CollaborativeAgentState) -> Dict[str, str]:
    ...
    system_message_writer = SystemMessage(
        content="Você é um agente escritor especialista. Sua tarefa é redigir um texto claro, coeso e bem estruturado com base na descrição da tarefa e no contexto fornecido (resultados de tarefas anteriores, se houver). Siga as instruções da descrição da tarefa cuidadosamente."
    )
    prompt_content = f"{context}Descrição da Tarefa Atual:\n{task_description}"
    human_message = HumanMessage(content=prompt_content)

    try:
        response = models["gpt_3_5"].invoke([system_message_writer, human_message])
        return {"specialist_result": response.content}
    except Exception as e:
...
def collect_result_and_advance_node(state: CollaborativeAgentState) -> Dict[str, Any]:
    ...
    Para simplificar, vamos fazer com que os nós especialistas retornem um dict com "specialist_result"
    e este nó será chamado após eles, pegando esse valor do estado.
    Alternativa: Modificar a forma como os nós são chamados ou usar `partial` para passar o resultado.
    ...
    return {
        ...
        "specialist_result": None # Limpa para a próxima iteração
    }
...
def synthesis_node(state: CollaborativeAgentState) -> Dict[str, str | None]:
    ...
    system_message_synthesis = SystemMessage(
        content="Você é um assistente de IA especialista em sintetizar informações. Sua tarefa é pegar a consulta original do usuário e os resultados de várias sub-tarefas executadas por agentes especialistas, e combiná-los em uma resposta final coesa, abrangente e bem escrita para o usuário. É CRUCIAL que você UTILIZE os 'Resultados das Sub-tarefas Executadas' como base principal para sua resposta. NÃO ignore esses resultados. Se os resultados parecerem incompletos ou vazios, mencione isso, mas ainda assim tente construir a melhor resposta possível com base no que foi fornecido e na consulta original."
    )
    human_message = HumanMessage(content=context)

    try:
        response = models["gpt_3_5"].invoke([system_message_synthesis, human_message])
...
collaborative_workflow.get_graph().draw_png("workflow.png")