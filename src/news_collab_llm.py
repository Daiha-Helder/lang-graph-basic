...
def planner_node(state: NewsAgentState) -> Dict[str, Any]:
    ...
    plan = [
        ...
        },
        {
            ...
            "description": "Analise o resumo da notícia, destacando pontos importantes, possíveis vieses e impacto social/político. Use o resultado da tarefa 'summarize_news'."
        },
        {
            "task_id": "suggest_questions",
            "specialist_type": "questioner",
            "description": "Sugira perguntas para reflexão ou debate baseadas na análise da notícia. Use o resultado da tarefa 'analyze_news'."
        }
    ]
    return {
        "plan": plan,
...
def summarizer_node(state: NewsAgentState) -> Dict[str, str]:
    ...
    try:
        response = models["gpt_3_5"].invoke([HumanMessage(content=prompt)])
